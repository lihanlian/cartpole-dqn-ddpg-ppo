...
class Cartpole:
    def __init__(self, cfg):
        ...
        # allocate buffers
        self.obs_buf = torch.zeros((self.cfg.num_envs, self.num_obs), device=self.cfg.sim_device)
        self.reward_buf = torch.zeros(self.cfg.num_envs, device=self.cfg.sim_device)
        self.reset_buf = torch.ones(self.cfg.num_envs, device=self.cfg.sim_device, dtype=torch.long)
        self.progress_buf = torch.zeros(self.cfg.num_envs, device=self.cfg.sim_device, dtype=torch.long)

        # acquire gym interface
        self.gym = gymapi.acquire_gym()
        self.sim = self.gym.create_sim(0, cfg.graphics_device_id, gymapi.SIM_PHYSX, sim_params)

        # initialise envs and state tensors
        self.envs, self.num_dof = self.create_envs()
        self.dof_states = self.get_states_tensor()
        self.dof_pos = self.dof_states.view(self.cfg.num_envs, self.num_dof, 2)[..., 0]
        self.dof_vel = self.dof_states.view(self.cfg.num_envs, self.num_dof, 2)[..., 1]
    ...
    def create_viewer(self):
        ...
        self.gym.viewer_camera_look_at(viewer, self.envs[self.cfg.num_envs // 2], cam_pos, cam_target)
        return viewer
    ...
    def get_states_tensor(self):
        ...
        dof_states = dof_states.view(self.cfg.num_envs, self.num_obs)
        return dof_states

    def get_obs(self, env_ids=None):
        # get state observation from each environment id
        if env_ids is None:
            env_ids = torch.arange(self.cfg.num_envs, device=self.cfg.sim_device)
    ...
    def reset(self):
        ...
        positions = 0.2 * (torch.rand((len(env_ids), self.num_dof), device=self.cfg.sim_device) - 0.5)
        velocities = 0.5 * (torch.rand((len(env_ids), self.num_dof), device=self.cfg.sim_device) - 0.5)
    ...
    def step(self, actions):
        # apply action
        actions_tensor = torch.zeros(self.cfg.num_envs * self.num_dof, device=self.cfg.sim_device)
